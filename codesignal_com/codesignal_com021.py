...
def solution(image):
    ...
    for row in range(1, len(image) - 1):
        ...
        for col in range(1, len(image[row]) - 1):
            element.append(int((sum(image[row-1][col-1:col+2]) + sum(
                image[row][col-1:col+2]) + sum(image[row+1][col-1:col+2])) / 9))
...
assert solution(image) == [[5, 4],
                           [4, 4]]

image = [[36, 0, 18, 9],
         [27, 54, 9, 0],
         [81, 63, 72, 45]]
assert solution(image) == [[40, 30]]
